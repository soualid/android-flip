  ...
  private boolean blockTouch = false;
	...
	public FlipViewController(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
				R.styleable.FlipViewController, 0, 0);

		try {
			int value = a.getInteger(R.styleable.FlipViewController_orientation,
					VERTICAL);
			if (value == HORIZONTAL) {
			...
			value = a.getInteger(
					R.styleable.FlipViewController_animationBitmapFormat, 0);
			if (value == 1) {
	...
	public void setSelection(int position) {
		...
		Assert.assertTrue("Invalid selection position", position >= 0
				&& position < adapterDataCount);
	...
	private void debugBufferedViews() {
		if (AphidLog.ENABLE_DEBUG) {
			AphidLog.d("bufferedViews: %s; buffer index %d, adapter index %d",
			...
			if (indexInAdapter == adapterIndex + 1) { // forward one page
				if (adapterIndex < adapterDataCount - 1) {
					...
					if (adapterIndex + sideBufferSize < adapterDataCount) {
						bufferedViews.addLast(viewFromAdapter(
								adapterIndex + sideBufferSize, true));
					}
				...
				AphidLog.e("Should not happen: indexInAdapter %d, adapterIndex %d",
						indexInAdapter, adapterIndex);
			}
		} else {
	...
	private void hideFlipAnimation() {
		...
		//if (inFlipAnimation) {
			...
			if (onViewFlipListener != null) {
				onViewFlipListener.onViewFlipped(bufferedViews.get(bufferIndex),
						adapterIndex);
			}
